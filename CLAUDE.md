# Goonj — Claude Code Instructions
...
dhvani (impulse responses for convolution reverb), shruti (room simulation for mixing), kiran/joshua (game audio propagation, occlusion), aethersafha (spatial audio for conferencing)

## Dependency Stack

```
goonj (acoustics)
  └── hisab (math — Vec3, geometry, spatial structures)
```
...
- **Own the stack.** Use hisab types (Vec3), not raw arrays long-term.
- **No magic.** Every operation is measurable, auditable, traceable.
- **`#[non_exhaustive]`** on all public enums.
- **`#[must_use]`** on all pure functions.
- **`#[inline]`** on hot-path functions (ray intersection, propagation, mode computation).
